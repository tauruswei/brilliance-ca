...
func (hs *serverHandshakeState) doFullHandshake() error {
	...
	var ski []byte           // ski for client auth, if any
	...
	if len(c.peerCertificates) > 0 {
		ski = hs.c.peerCertificates[0].SubjectKeyId
		msg, err = c.readHandshake()
		if err != nil {
			return err
		}
		...
		case *ecdsa.PublicKey:
			...
			switch key.Curve {
			case sm2.P256Sm2():
				//	fmt.Println("验证签名 sm2.P256Sm2() doFullHandshake")
				if hs.c.config.Verifier != nil {
					//fmt.Println("=================================")
					//fmt.Println("验签:server doFullHandshake")
					//fmt.Println(hs.c.peerCertificates[0].Subject.CommonName)
					//fmt.Println("=================================")
					_, err := hs.c.config.Verifier.Verify(ski, certVerify.signature, digest)
					if err != nil {
