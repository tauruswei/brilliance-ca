	...
	for _, e := range in.TBSCertificate.Extensions {
		...
		if len(e.Id) == 4 && e.Id[0] == 2 && e.Id[1] == 5 && e.Id[2] == 29 {
			...
			case 60000:
				// RFC 5280, 4.2.1.2
				var netWorkId []byte
				if rest, err := asn1.Unmarshal(e.Value, &netWorkId); err != nil {
					return nil, err
				} else if len(rest) != 0 {
					return nil, errors.New("x509: trailing data after X.509 network name")
				}
...
var (
	...
	oidExtensionNetWorkId             = []int{2, 5, 29, 60000}
...
func buildExtensions(template *Certificate) (ret []pkix.Extension, err error) {
	...
	if len(template.NetWorkId) > 0 && !oidInExtensions(oidExtensionNetWorkId, template.ExtraExtensions) {
		ret[n].Id = oidExtensionNetWorkId
		ret[n].Value, err = asn1.Marshal(template.NetWorkId)
		if err != nil {
			return
		}
		n++
...
type CertificateRecord struct {
	Serial    *big.Int  `db:"serial_number"` // 证书序列号
	AKI       string    `db:"authority_key_identifier"`
	CALabel   string    `db:"ca_label"`
	Status    string    `db:"status"`
	Reason    int       `db:"reason"`
	Expiry    time.Time `db:"expiry"`     // 过期的时间段
	RevokedAt time.Time `db:"revoked_at"` // revoke 开始时间
	PEM       string    `db:"pem"`
}
