...
//调用SM2接口生成SM2证书
func CreateCertificateToMem(template, parent *sm2.Certificate, signer interface{}) (cert []byte, err error) {
	...
	if a {
		...
		puk.X = pub.X
		puk.Y = pub.Y
		cert, err = sm2.CreateCertificateToMem(template, parent, &puk, signer)
	}
	return
}
