...
func LoadPrivateKey(keystorePath string) (bccsp.Key, crypto.Signer, error) {
	...
	walkFunc := func(path string, info os.FileInfo, err error) error {
		if strings.HasSuffix(path, "_sk") {
			...
			priv, err = csp.KeyImport(rawKey, &bccsp.GMSM2PrivateKeyImportOpts{Temporary: true})
			if err != nil {
				return err
			}
...
// LoadTlsPrivateKey loads a private key from file in keystorePath
...
func LoadRootPrivateKey(keystorePath string) (bccsp.Key, crypto.Signer, error) {
	var err error
	var priv bccsp.Key
	var s crypto.Signer
...
// GeneratePrivateKey creates a private key and stores it in keystorePath
func GetPrivateKey(keystorePath string, ski []byte) (bccsp.Key,
	crypto.Signer, error) {
...
// GenerateRootPrivateKey creates a private key and stores it in keystorePath
func GenerateRootPrivateKey(keystorePath string) (bccsp.Key, crypto.Signer, error) {
